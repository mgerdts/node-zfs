...
tap.test('Dataset', (tt) => {
    ...
    tt.test('root dataset', (t) => {
        ...
        all.add(other.name);
        check_verror(t, function () { new Dataset(null, 'root', 'filesystem') },
            'DatasetExistsError', 'do not allow duplicate pools');
        check_verror(t, function () { new Dataset(null, 'foo', 'filesystem') },
            'DatasetExistsError', 'do not allow duplicate pools');
        ...
        for (var ds of root.iterDescendants(['all'])) {
            t.ok(ds.name.startsWith('root'), `${ds.name} is in the foo pool`);
            t.ok(all.delete(ds.name), `${ds.name} was found`);
        }
        for (ds of other.iterDescendants(['all'])) {
            t.ok(ds.name.startsWith('foo'), `${ds.name} is in the foo pool`);
            t.ok(all.delete(ds.name), `${ds.name} was found`);
        }
        ...
        t.doesNotThrow(function () { other.destroy(); }, 'zfs destroy foo');
    ...
    tt.test('dataset names', (t) => {
        ...
        }, 'DatasetNameError', 'dataset name cannot be an empty string');
        ...
        t.doesNotThrow(function () {
            let ds = new Dataset(root, 'a'.repeat(255), 'filesystem');
            ds.destroy();
    ...
    tt.test('dataset iterator', (t) => {
        ...
        function check(types, top) {
            ...
            for (ds of top.iterDescendants(types)) {
                t.ok(!found.has(ds), `${ds.name} not previously found`);
            ...
            for (ds of expect) {
                t.ok(false, `${ds.name} was expected but not found`);
            }
        }
        ...
        found = new Set();
        expect = new Set([
            root, snap1, fs1, fs1a, fs1v, snap2, snap2a, snap2v, snap3 ]);
        check(['all']);
        ...
        found = new Set();
        expect = new Set([
            root, snap1, fs1, fs1a, fs1v, snap2, snap2a, snap2v, snap3, fs2 ]);
        check(['all']);
        ...
        found = new Set();
        expect = new Set([ fs1, fs1a, fs1v, snap2, snap2a, snap2v, snap3 ]);
        check(['all'], fs1);
    ...
    tt.test('snapshot', (t) => {
        ...
        check_verror(t, function () { root.snapshot('snap2'); },
            'DatasetExistsError',
            'cannot create a second snapshot of the same name after rename');
    ...
    tt.test('clone', (t) => {
        ...
        check_verror(t, function () { fs1.clone('root/noway'); },
            'DatasetTypeError', 'cannot clone a filesystem (only snapshot)');
        ...
        t.doesNotThrow(function () { fs2.destroy(); }, 'can destroy root/fs2');
        t.doesNotThrow(function () {
            fs2 = snap1.clone('root/fs2', {}, { mountpoint: '/blah' } );
        }, 'zfs clone root/fs1@snap1 root/fs2, with custom mountpoint');
        t.equal(fs2.mountpoint, '/blah', 'root/fs2 mountpoint is /blah');
        ...
        check_verror(t, function () { vol1.destroy(); }, 'DescendantError',
        ...
        check_verror(t, function () { vol1.destroy({recursive: true}); },
            'DependantError', 'cannot recursive destroy a volume with a clone');
    ...
    tt.test('holds', (t) => {
        ...
        check_verror(t, function () { fs1.hold('nope'); }, 'DatasetTypeError',
        ...
        for (let ds of fs1.iterDescendants(['snapshot'])) {
            ...
            t.ok(ds.holds().has('reason1'), `${ds.name} has reason1 hold`);
        }
        ...
        check_verror(t, function () { fs1a.destroy({ recursive: true }); },
            'SnapshotHoldError',
            'cannot recursively destroy filesystem with a held snapshot');
    ...
    tt.test('properties', (t) => {
        ...
        let rw_inheritable_props = [
            ...
            [ 'checksum', [ 'on', 'off' ] ],    // Other values legal, but meh.
            [ 'compression', [ 'on', 'off' ] ], // Other values legal, but meh.
            [ 'copies', [ 1, 2, 3 ] ]
        ];
        ...
        for (prop in rw_inheritable_props) {
            ...
            for (ds in datasets) {
                ...
                t.doesNotThrow(getval, `can get ${prop} on ${ds.name}`);
                t.ok(values.includes(val), `${val} is one of ${values}`);
            }
            ...
            t.equals(root[prop], defval,
                `${prop}=${localval} on fs1 did not change value on root`);
            t.equals(fs1[prop], localval,
                `${prop}=${localval} on fs1 changed value on fs1`);
            t.equals(fs2[prop], localval,
                `${prop}=${localval} on fs1 changed value on fs2`);
        }
        ...
        for (prop in ro_props) {
            ...
            for (ds in datasets) {
                ds = datasets[ds];
                t.doesNotThrow(getval, `can get ro prop ${prop} on ${ds.name}`);
    ...
    tt.test('catch new implementations', (t) => {
        // Nag anyone that implements these.
        let props = [ 'aclinherit', 'acltype', 'casesensitivity', 'context',
            'dedup', 'defcontext', 'devices', 'dnodesize', 'encryption', 'exec',
            'filesystem_count', 'filesystem_limit', 'fscontext', 'keyformat',
            'keylocation', 'logbias', 'logicalreferenced', 'logicalused',
            'mlslabel', 'nmbmand', 'normalization', 'objsetid', 'overlay',
            'pbkdf2iters', 'primarycache', 'quota', 'readonly', 'recordize',
            'redundant_metadata', 'refcompressratio', 'referenced', 'refquota',
            'refreservation', 'relatime', 'reservation', 'rootcontext',
            'secondarycache', 'setuid', 'sharenfs', 'sharesmb', 'size',
            'snapdev', 'snapdir', 'snapshot_count', 'snapshot_limit',
            'special_small_blocks', 'sync', 'usedbychildren', 'usedbydataset',
            'usedbyrefreservation', 'usedbysnapshots', 'utf8only', 'version',
            'volblocksize', 'volmode', 'vscan', 'written', 'xattr', 'zoned' ];
        ...
        for (let prop in props) {
            ...
            try {
                root[prop] = 1;
                t.ok(false, `unexpectedly able to set '${prop}'`);
            } catch (err) {
            ...
            try {
                let foo = root[prop]
                t.ok(false, `unexpectedly able to get '${prop}' (${foo})`);
            } catch (err) {
                t.equal(err.message, 'not implemented',
