 ...
 * the filesystems to be mounted within one of the roots passed to
 * mockds.reset().  Filesystems can be mounted and unmounted with
 * Dataset.mount() and Dataset.unmount().
 *
 * When a filesystem is unmounted or snapshotted, the content of that dataset is
 * archived and stored with the Dataset object.  When a snapshot of a filesystem
 * is cloned, the filesystem references the snapshot's archive.  When a
 * filesystem is mounted, the archived data is restored to the mock filesystem
 * and the archive is deleted (reference removed, may still exist on origin
 * snapshot.)
 */
...
// Dataset operations that would normally cause file system objects to appear
// will be carried out in the directories under the paths in this array.
var roots;
 ...
 * @param {string[]} roots - Directories under which file system operations
 * should take place.  Writes to the file system happen during Dataset.mount()
 * and Dataset.unmount().  Reads happen during Dataset.snapshot() if the dataset
 * is mounted.
 */
function reset(fsroots) {
    assert.optionalArrayOfString(fsroots);

    // Be a little paranoid about which roots are allowed because umount will do
    // the equivalent of rm -rf within them.  If they aren't mocked, this could
    // lead to unexpected damaage.
    for (var root in fsroots) {
        root = fsroots[root];
        assert(root.startsWith('/'), 'starts with "/"');
        assert(root !== '/', 'must not be "/"');
        assert(root.indexOf('//') === -1, 'does not have "//"');
        assert(root.split('/').indexOf('..') === -1, 'does not have ".."');
        assert(root.split('/').indexOf('.') === -1, 'does not have "."');
    }
    roots = fsroots || [];
...
/**
 * Get the filesystem that contains the specified path.
 * @param {string} name - the name of the file or directory
 * @returns {Dataset}
 * @throws {VError} if there is no mounted dataset with that path.
 */
function fsFromPath(name) {
    var longest = '';

    for (var mntpt in mnttab) {
        if (name === mntpt || name.startsWith(mntpt + '/')) {
            if (mntpt.length > longest.length) {
                longest = mntpt;
            }
        }
    }
    ...
    if (longest === '') {
        throw new VError({ info: name, name: 'NoDatasetFoundError' },
            'could not find a filesystem that contains \'%s\'', name);
    }

    return mnttab[longest];
}
 ...
 * restore().
 */
function archive(opts, name) {
    assert.string(name, 'name');
    assert(isInRoot(name), 'name is under one of the roots');

    var thisfs = fsFromPath(name);
    var obj = {};

    obj.name = path.basename(name);
    obj.lstat = fs.lstatSync(name);

    if (obj.lstat.isFile()) {
        assert(obj.lstat.size <= 1024 * 1024, 'file size limited to 1 MiB');
        obj.filedata = fs.readFileSync(path);
        return obj;
    }
    ...
    if (obj.lstat.isDirectory()) {
        ...
        for (var kid in kids) {
            kid = kids[kid];
            var kpath = path.join(name, kid.name);
            // Do not cross mount points.
            if (mnttab[kpath]) {
                continue;
            }
            obj.children.push(archive(kname));
        }
        return obj;
    }
...
function restore(basedir, obj) {
    ...
    assert(isInRoot(basedir), 'basedir is within a root');
...
function clearDir(dir) {
    ...
    assert(isInRoot(dir), 'removing files from a test root');

    // mock-fs module sets the dev to Jenny's phone number. :)
    assert.equal(fs.statSync(dir).dev, 8675309, 'should be a mock-fs dir');

    var children = fs.readdirSync(dir);
    for (var child in children) {
        child = children[child];
        var cpath = path.join(dir, cpath);
    ...
    * iterDescendants(types, state) {
    ...
    mount(opts) {
        ...
        try {
            ...
            fs.mkdirSync(self.mountpoint);
        }

        mnttab[self.mountpoint] = self;
        self._mounted = true;
        if (self._fscontent) {
            restore(self.mountpoint, self.fs_content);
        }
        self._fscontent = null;
    }
    ...
    snapshot(snapname, opts, properties) {
        ...
        if (self._parent._fscontent) {
            // The filesystem was previously mounted, now unmounted.
            self._fscontent = self._parent._fscontent;
        } else if (self._parent._mounted) {
            self._fscontent = archive(self._parent.mountpoint);
        }
