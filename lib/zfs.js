...
function diskinfoLinux(callback) {
	execFile('lsblk', [ '--json', '--bytes', '--output', '+rota' ],
			{ timeout: timeoutDuration }, function (error, stdout, stderr) {
		if (error) {
		...
		try {
			...
			callback(new Error('Unable to parse lsblk outout: ' + ex));
			return;
		}

		disks = result.blockdevices.filter(function _filterDisks(d) {
			return d.type === 'disk';
	...
	execFile(exports.paths.zpool, [ 'status', pool ],
		...
		for (var i = 0; i < lines.length; i++) {
			if (lines[i].trim().substr(0, 5) === 'state') {
				return (callback(null, lines[i].trim().substr(7)));
			}
		}
		callback(null, 'UNKNOWN');
	});
