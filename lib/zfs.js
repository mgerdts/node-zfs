...
function diskinfoSunOS(callback) {
...
function diskinfoLinux(callback) {
	execFile('lsblk', [ '--json', '--bytes', '--output', '+rota' ],
		...
		try {
			...
			callback(new Error(
				'Unable to parse lsblk outout: ' + ex));
			return;
		}

		var disks = result.blockdevices.filter(
				function _filterDisks(d) {
			return (d.type === 'disk');
		}).map(function _mapBlockDevices(d) {
			// Example d:
			// {
			//   "name": "sda",
			//   "maj:min": "8:0",
...
zpool.listDisks = function (callback) {
    ...
    if (osType === 'SunOS') {
		diskinfoSunOS(callback);
	} else if (osType === 'Linux') {
		diskinfoLinux(callback);
	} else {
...
zpool.list = function () {
	...
	var args = ['list', '-H', '-o', opts.fields.join(',')];
	if (opts.parseable) {
		args.push('-p');
	}
	...
	execFile(exports.paths.zpool, args, { timeout: timeoutDuration },
		...
		var rows = parseTabSeperatedTable(stdout);
		return (callback(null, opts.fields, rows));
	});
};
...
zfs.get = function (name, propNames, parseable, callback) {
	...
	var stdout = '', stderr = '';
	var child = spawn(exports.paths.zfs, argv);
...
zfs.clone = function () {
	...
	for (var prop in properties) {
		args.push('-o', prop + '=' + properties[prop]);
	}
	args.push(snapshot, name);

	execFile(exports.paths.zfs, args, { timeout: timeoutDuration },
	    function (error, stdout, stderr) {
		if (error) {
			callback(zfsError(error, stderr));
			return;
		}
		callback();
	});
...
zfs.list = function () {
	...
	options.fields    = options.fields || zfs.listFields_;
	options.parseable = options.parseable || false;

	var args = [ 'list', '-H', '-o', options.fields.join(','),
	    '-t', options.type ];
	...
	execFile(exports.paths.zfs, args, { timeout: timeoutDuration },
		...
		if (error)
			return (callback(zfsError(error, stderr)));
		var rows = parseTabSeperatedTable(stdout);
		return (callback(null, options.fields, rows));
	});
};
...
zfs.hold = function (snapshot, tag, callback) {
	if (arguments.length != 3)
		throw new Error('Invalid arguments');

	var args = [ 'hold', tag, snapshot ];
	execFile(exports.paths.zfs, args, { timeout: timeoutDuration },
	    function (error, stdout, stderr) {
		if (error) {
			callback(new Error(stderr.toString()));
...
zfs.releaseHold = function (snapshot, tag, callback) {
	if (arguments.length != 3)
		throw new Error('Invalid arguments');

	var args = [ 'release', tag, snapshot ];
	execFile(exports.paths.zfs, args, { timeout: timeoutDuration },
	    function (error, stdout, stderr) {
		if (error) {
			callback(new Error(stderr.toString()));
