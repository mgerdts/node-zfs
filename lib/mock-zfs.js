...
const Dataset = require('./mock-dataset.js').Dataset;
...
zpool.status = function (pool, callback) {
    ...
    if (Dataset.getPools().indexOf(pool) === -1) {
...
zfs.create = function (name, callback) {
    ...
    try {
        var ma = Dataset.get(getParentName(name));
    } catch (err) {
...
zfs.get = function (name, propNames, parseable, callback) {
    ...
    try {
        ...
        let pools = Dataset.getPools();
        for (let pool in pools) {
            root = Dataset.get(dataset[pool]);
